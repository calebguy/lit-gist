...
const _litActionCode = async () => {
  try {
    ...
    const combinedRandomness = Array.from(new Uint8Array(combinedSignature))
      .map((b) => b.toString(16).padStart(2, "0"))
      .join("")
    ...
    const finalRandomnessHex = `0x${Array.from(new Uint8Array(finalRandomness))
      .map((b) => b.toString(16).padStart(2, "0"))
      .join("")}`
    ...
    const hashedTxToSign = ethers.utils.arrayify(
